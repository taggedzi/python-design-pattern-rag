...
def enhance_with_ollama(code: str, model: str = 'pattern-rag-gen:latest', retries: int = 3, delay: float = 2.0) -> Optional[str]:
    try:
        import ollama
    except ImportError:
    ...
    if not hasattr(ollama, "chat"):
        raise RuntimeError("The `ollama` package is installed but missing the `chat` method. Check your version or upgrade it.")
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                raise RuntimeError(f"Ollama enhancement failed: {e}")
...
def clean_and_chunk_repo(source_dir, output_dir, enhance=False, keep_original=False, skip_enhanced=False, model="pattern-rag-gen:latest"):
    ...
    for file_path in source_dir.rglob("*.py"):
        ...
        base_name = rel_path.with_suffix('').as_posix().replace('/', '_').replace('\\', '_')
...
if __name__ == "__main__":
    ...
    parser.add_argument("source", help="Path to the Python project directory")
